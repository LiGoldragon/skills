...
String boundaries live in dedicated boundary or adapter components, not in core daemon machinery. HTTP/JSON provider calls and model-text or NOTA parsing belong to a component whose charter is that boundary, which returns typed signals inward. Do not fold a string boundary into an agent daemon whose job is state and logic.
...
When a daemon needs model judgment or another text-mediated service, put prompts, raw model text parsing, and text diagnostic artifacts in an adapter or client process. That process returns a typed rkyv signal to the daemon. The `message` component is the reference text-ingress boundary; the judge adapter daemons are the reference model-text boundary.
