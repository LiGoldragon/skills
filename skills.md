...
- Keep supported model families and efforts in `manifests/model-catalog.nota`, exactly one model profile per active role in `manifests/role-model-assignments.nota`, and exactly one optional-skill record per active role in `manifests/role-optional-skills.nota`.
...
- Update hardcoded generation expectations in `tests/generation.rs` when active module membership, role composition, profiles, optional skills, or universal role modules intentionally change.
