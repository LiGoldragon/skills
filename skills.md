...
- Keep supported model+effort profiles and their explicit total-order strengths in `manifests/model-catalog.nota`, exactly one model profile per active role in `manifests/role-model-assignments.nota`, exactly one optional-skill record per active role in `manifests/role-optional-skills.nota`, and Pi-only execution budgets in `manifests/role-execution-budgets.nota`.
...
`general-instructions` on `CodexAgent`. It has no active `PiAgent` insertion.
