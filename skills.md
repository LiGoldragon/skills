...
- Keep models, closed capability tiers, and supported efforts in `manifests/model-catalog.nota`; compare profiles lexicographically by tier then effort. Keep exactly one model assignment per active role.
- Keep direct optional-skill records in `manifests/role-optional-skills.nota` and typed nested-role minimum models and exclusive leaf edges in `manifests/nested-role-relations.nota`.
- Put genuinely shared preloaded modules, optional skills, and child policy in `manifests/role-curricula.nota`; assigned roles leave those direct fields empty so target-specific identities cannot drift.
...
- Put harness-specific frontmatter metadata in schema-authored typed records, manifests, or the compatibility roster, not in reusable module prose. Pi `allowedChildRoleNames` is a YAML sequence.
